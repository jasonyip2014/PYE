...
class Amasty_Promo_Model_Observer
{
    ...
    public function handleValidation($observer) 
    {
		...
		foreach ($promoSku as $sku){
            ...
            if ($this->_addProductToQuote($quote, $product, $qty, $rule)){
                Mage::log($product->debug(),null,"test_free_gift.log");
    ...
    public function saveExtraGift($observer)
    {
        ...
        foreach($items as $item) {
            //Mage::log($item->getName(),null,"test20141006.log");
            if($item->getPrice() == 0 && strpos($item->getName(),$prefix) !== false) {
                $extraGift[] = $item->getItemId();
            }
        }
        Mage::getSingleton('core/session')->setExtraGift($extraGift);
        //Mage::log($extraGift,null,"test20141006.log");
    }

    public function removeExtraGift($observer)
    {
        $cartHelper = Mage::helper('checkout/cart');
        $action = Mage::app()->getFrontController()->getAction();
        if ($action->getFullActionName() == 'sales_order_reorder')
        {
            $extraGift  =  Mage::getSingleton('core/session')->getExtraGift();
            //Mage::log($extraGift,null,"test20141006.log");
            foreach($extraGift as $itemId) {
                $cartHelper->getCart()->removeItem($itemId)->save();
            }
            Mage::getSingleton('core/session')->unsExtraGift();
        }
    }
}